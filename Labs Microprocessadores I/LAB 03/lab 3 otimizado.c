...
void escreve_dado(char dado)
{
	...
	E=0;
	P1 &= 0x0F;						// Limpa porta 1
	P1 |= (dado << 4);		// Esreve o dado nos pinos ainda nao preenchido da porta 1
	E=1;									// Sobre e desce o enable para realizar a escrita
	E=0;
...
void escreve_comando(char dado)
{
	...
	P1 &= 0x0F;						// Limpa 4 bits superiores da porta 1
	P1 |= (dado & 0xF0);	// Ativa bits na parte alta do comando
	E=1;									// Sobre e desce o enable para realizar a escrita
	E=0;
	P1 &= 0x0F;						// Limpa 4 bits superiores da porta 1
	P1 |= (dado << 4);		// Ativa bits na parte baixa do comando
	E=1;									// Sobre e desce o enable para realizar a escrita
	E=0;
	atraso_lcd();					// Chama o atraso para tudo ocorrer corretamente
}
...
char converte_num(char num)	// Conversão de digitos de 0 a 9 em seus correspondetes como caracteres
{
	switch(num)
	{
...
void escreve_num(char num, char*vetor)
{
	...
	vetor[2] = 32;	// Caractere de espaço
}
...
void main ()
{
	...
	for (i=0; i<10; i++) // percorre as linhas da matriz
	{
		for (j=0; j<10; j++) // percorre as colunas da matriz
		{
			...
			{
				num = num + MEDICOES[i][j];				// soma o numerador da fracao para calculo da media
				den++;
			}
